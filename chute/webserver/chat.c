...
int UDPsetupConnection(int port)
{
	int connectionSock;
	socklen_t addr_len;
        ...
        // add tcp_ nodelay option for avoiding socket error

        if((connectionSock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1)
        {
                printf("ERROR:UDP Socket creation error.\n");
                exit(1);
        }
          ...
          sender_addr.sin_addr.s_addr = htons(INADDR_ANY);

          bcopy((char *)host->h_addr, (char *)&sender_addr.sin_addr.s_addr, host->h_length);
	
	if(bind(connectionSock, &sender_addr, sizeof(sender_addr)) <0)
	{
		 printf("ERROR(%s) binding to socket\n",strerror(errno));
		return -1;
	}
...
int setupConnection(int port)
{
  ...
  returnVal = connect(connectionSock,(struct sockaddr *) &sender_addr,sizeof(sender_addr));
  if(returnVal < 0)
	{
		printf("ERROR(%s) connecting to socket\n",strerror(errno));
		return -1;
	}
...
//  int connectionSock = setupConnection(5000);
  int connectionSock = UDPsetupConnection(5000);
  if(connectionSock < 0)
   {
